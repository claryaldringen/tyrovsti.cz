import { Col, Row } from 'reactstrap'
import React from 'react'
import { Qt } from '../../../Quote/Qt'
import { PUBLICATIONS } from '../../../../shared/constants'
import { ArticleProps } from '../../../../types'
import { DraftBadge } from '../../../DraftBadge'

export const HlohovskyDedickySpor = ({ draft }: ArticleProps) => (
  <>
    <Row>
      <Col>
        <div className="anchor" id="hlohovsky-dedicky-spor" />
        <h4>Hlohovský dědický spor (1476–1488){draft && <DraftBadge />}</h4>
        <p>
          Hlohovský dědický spor (něm. <i>Glogauer Erbfolgestreit</i>, pol.{' '}
          <i>Wojna o sukcesję głogowską</i>) byl ozbrojený konflikt
          o&nbsp;slezské knížectví Hlohovské, k&nbsp;němuž došlo po smrti vévody
          Jindřicha&nbsp;XI. Hlohovského roku 1476. Hlavními aktéry byli vdova
          po Jindřichovi Barbora Braniborská a&nbsp;její otec, kurfiřt
          Albrecht&nbsp;III. Achilles Braniborský, na straně jedné,
          a&nbsp;hlohovský bratranec zesnulého{' '}
          <b>Jan&nbsp;II. Hlohovský zvaný Šílený</b>, na straně druhé. Spor se
          ihned protkl s&nbsp;velkou politikou, neboť do něj vstoupili Matyáš
          Korvín a&nbsp;obě hlavy Jagellonského domu.
          <Qt
            publication={PUBLICATIONS.WIKI}
            href="https://de.wikipedia.org/wiki/Glogauer_Erbfolgestreit"
          />
        </p>
      </Col>
    </Row>
    <Row>
      <Col>
        <h5>Předehra</h5>
        <p>
          Po smrti Jindřicha&nbsp;XI. <b>22. února 1476</b> nechtěl Jan&nbsp;II.
          ustoupit braniborským nárokům. Albrecht&nbsp;III. Achilles získal pro
          svou dceru Barboru souhlas Matyáše Korvína, který jako uherský král
          vykonával lenní svrchovanost nad Slezskem od olomoucké dohody 1469.
          Jan&nbsp;II. oproti tomu hledal podporu u&nbsp;jagellonského dvora;
          jeho mladší bratr Václav Hlohovský byl ostatně dlouholetým služebníkem
          Vladislava Jagellonského. Konflikt tak rychle nabyl mezinárodního
          rozměru.
          <Qt
            publication={PUBLICATIONS.WIKI}
            href="https://pl.wikipedia.org/wiki/Wojna_o_sukcesj%C4%99_g%C5%82ogowsk%C4%85"
          />
        </p>
        <h5>Průběh</h5>
        <p>
          Jan&nbsp;II. obsadil v&nbsp;létě 1476 město Hlohov, vyhnal Barboru
          Braniborskou z&nbsp;její vdovské rezidence a&nbsp;zmocnil se klíčových
          hradů knížectví — Krosna, Świebodzina, Zelené Hory (Zielonej Góry)
          a&nbsp;Šprotavy (Szprotawy). Albrecht Achilles odpověděl vojenskou
          výpravou v&nbsp;roce 1477; braniborská a&nbsp;saská vojska oblehla
          Krosno a&nbsp;Sulechów, ale přímý úspěch jim unikl. Obě strany sahaly
          k&nbsp;drancování slezského venkova a&nbsp;do bojů se zapojili také
          hejtmani z&nbsp;polské strany — mezi nimi Jakub z&nbsp;Dębna
          a&nbsp;Mikuláš z&nbsp;Kurozwęk.
          <Qt
            publication={PUBLICATIONS.WIKI}
            href="https://de.wikipedia.org/wiki/Glogauer_Erbfolgestreit"
          />
        </p>
        <p>
          Spor se vlekl řadu let. Roku 1480 dosáhl Jan&nbsp;II. dílčího
          vyrovnání s&nbsp;braniborskou stranou (Kamenecká dohoda), jímž si
          potvrdil držbu jádra knížectví výměnou za vyplacení odstupného
          Barboře. Po smrti Albrechta Achilla roku 1486 se však konflikt znovu
          rozhořel. Matyáš Korvín tentokrát zasáhl rozhodným způsobem: roku{' '}
          <b>1488</b> vyslal do Slezska oddíly své armády, Jana&nbsp;II.
          Hlohovského zajal, uvěznil ho na Hlohově a&nbsp;uvalil na knížectví
          přímou královskou správu. Jan&nbsp;II. Hlohovský zemřel v&nbsp;zajetí{' '}
          <b>22. dubna 1504</b>.
          <Qt
            publication={PUBLICATIONS.WIKI}
            href="https://pl.wikipedia.org/wiki/Wojna_o_sukcesj%C4%99_g%C5%82ogowsk%C4%85"
          />
        </p>
        <h5>Výsledek</h5>
        <p>
          Hlohovský dědický spor skončil trojnásobnou prohrou: pro Jana&nbsp;II.
          se stal osobní katastrofou, Hohenzollernové ztratili šanci připojit
          knížectví ke svému rodovému dědictví a&nbsp;samotným slezským
          Piastovcům se podobné dynastické krize opakovaly v&nbsp;dalších
          generacích. Z&nbsp;dlouhodobého hlediska posílil spor královskou
          pravomoc Matyáše Korvína nad Slezskem; po jeho smrti roku 1490 Hlohov
          připadl Vladislavovi Jagellonskému jako součást zemí Koruny české.
          <Qt
            publication={PUBLICATIONS.WIKI}
            href="https://de.wikipedia.org/wiki/Glogauer_Erbfolgestreit"
          />
        </p>
        <h5>Primární prameny</h5>
        <p>
          Klíčovým slezským vyprávěcím pramenem je <b>Peter Eschenloer</b>,
          městský písař vratislavský, jenž ve své{' '}
          <i>Geschichte der Stadt Breslau</i> (originál ukončen kolem 1481)
          podává očité svědectví o&nbsp;rivalitě Korvína s&nbsp;Jagellonci,
          slezské válce 1474 a&nbsp;počátcích hlohovského sporu. Edice G. Roth,
          Münster 2003; starší vydání J. G. Kunisch, Vratislav 1827–1828.
          <Qt
            publication={PUBLICATIONS.WIKI}
            href="https://archive.org/search?query=Eschenloer+Geschichte+Breslau"
          />
        </p>
        <p>
          Uherský pohled a&nbsp;Korvínovu intervenci roku 1488 zachycuje{' '}
          <b>Antonio Bonfini</b> v&nbsp;<i>Rerum Ungaricarum decades</i> (decas
          IV, lib. VIII–IX). Doplňující diplomatický materiál — zejména listiny
          mezi Hohenzollerny a&nbsp;Janem&nbsp;II. — je shromážděn v&nbsp;
          <i>Codex diplomaticus Silesiae</i>, sv. 9 (Vratislav 1869).
          <Qt
            publication={PUBLICATIONS.BONFINI}
            href="https://archive.org/details/bub_gb_NBPp2aOaL0AC"
          />
        </p>
        <h5>Účast českých žoldnéřů</h5>
        <p>
          Slezské bojiště přitahovalo žoldnéře z&nbsp;celé střední Evropy
          a&nbsp;české oddíly tu v&nbsp;70. a&nbsp;80. letech 15. století
          působily prakticky nepřetržitě. Na straně Matyáše Korvína bojovaly
          v&nbsp;rámci <i>Černé armády</i> roty pod českými hejtmany, mezi nimi
          Jan Haugvic z&nbsp;Biskupic a&nbsp;František z&nbsp;Háje; právě jejich
          rychlá akce vedla k&nbsp;zajetí Jana&nbsp;II. roku 1488. Na straně
          Jana&nbsp;II. Hlohovského působili v&nbsp;různých etapách polští
          žoldnéři i&nbsp;drobní šlechtici z&nbsp;Lužice a&nbsp;slezských
          knížectví, mezi nimiž byli i&nbsp;jednotliví čeští rotmistři. Konflikt
          je dobrým příkladem dynastického sporu, v&nbsp;němž se čeští žoldnéři
          ocitli na obou stranách současně.
          <Qt
            publication={PUBLICATIONS.WIKI}
            href="https://de.wikipedia.org/wiki/Glogauer_Erbfolgestreit"
          />
        </p>
      </Col>
    </Row>
  </>
)
