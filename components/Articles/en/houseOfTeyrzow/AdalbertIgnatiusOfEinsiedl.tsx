import { Col, Row } from 'reactstrap'
import React from 'react'
import { PUBLICATIONS } from '../../../../shared/constants'
import { Qt } from '../../../Quote/Qt'

export const AdalbertIgnatiusOfEinsiedl = () => (
  <>
    <Row>
      <Col>
        <div className="anchor" id="adalbert-ignatius-of-einsiedl" />
        <h4>
          Vojtěch Ignác (Adalbert Ignatius) Týřovský of Einsiedl (died 1695) and
          the end of the family at Chříč
        </h4>
        <p>
          Vojtěch Ignác inherited the Chříč estate with the town of Kožlany,
          burdened with considerable debts, while still a minor; his mother Eva
          Eurosina, née of Klenové, remained his guardian. In 1655 she sent the
          archbishop a certificate that her entire family and all the subjects
          in Kožlany were of the Catholic religion and that on Easter Monday
          they had attended divine service and the sermon delivered by the
          parish priest of Čistá, Jan Procerius, who also administered the
          parish of Kožlany.
          <Qt
            publication={PUBLICATIONS.KOCKA1}
            href="https://kramerius5.nkp.cz/view/uuid:cbd24070-4608-11ed-b64c-005056827e52?page=uuid:b80db396-2bc6-4d88-ae38-d19072b4ef54"
            note="p. 377"
          />
        </p>
        <p>
          He came of age in 1662 and took possession of his father&apos;s
          inheritance himself. Oto Jiří of Helversen, lord of Krakovec, at once
          secured on his estate 3,000 guilders which the young Týřovský owed him
          for gold objects, diamonds and rubies. In 1676 he additionally bought
          the village of Třímaný, and in the same year dismissed from his
          service the burgrave Daniel Vlašimský, formerly a Prague burgher, to
          whom he remained in debt for 500 guilders.
          <Qt
            publication={PUBLICATIONS.KOCKA1}
            href="https://kramerius5.nkp.cz/view/uuid:cbd24070-4608-11ed-b64c-005056827e52?page=uuid:982a126d-a074-4939-802f-22a8843d76ad"
            note="p. 316, 378"
          />
        </p>
        <p>
          He attempted to bring the people of Kožlany into complete subjection:
          he asked them to hand over all their privileges for inspection, and
          when after several years he did not return them, the burghers saw
          through their lord&apos;s ruse and complained to the Bohemian chamber.
          On the order of the regional captain he returned their charters to
          them in 1680. During his rule seven Kožlany subjects abandoned their
          holdings and fled. When the people of Čistá claimed annual fairs and a
          toll in 1680, the people of Kožlany opposed both the fairs and the
          toll, and their hereditary lord maintained that Břežanská hora
          belonged to him, since his ancestors had bought it from Albrecht
          Jindřich of Kolovrat.
          <Qt
            publication={PUBLICATIONS.KOCKA1}
            href="https://kramerius5.nkp.cz/view/uuid:cbd24070-4608-11ed-b64c-005056827e52?page=uuid:1ab20b93-66e8-48df-ae4b-67177e424fcd"
            note="p. 378, 473"
          />
        </p>
        <p>
          To the church at Milíčov, where his grandfather lay, he had a tower
          added in 1665 and provided an altar.
          <Qt
            publication={PUBLICATIONS.KOCKA1}
            href="https://kramerius5.nkp.cz/view/uuid:cbd24070-4608-11ed-b64c-005056827e52?page=uuid:6bf7328e-a9a9-4cb4-b5fc-b8c991028766"
            note="p. 411"
          />
        </p>
        <p>
          He died in 1695. He left behind a widow, Kateřina Magdalena, née
          Countess of Brandenburg, the sons Jan Vilém, Michal Antonín and
          František Pavel, and the daughters Anna and Polyxena. Chříč was
          inherited by Michal Antonín, who was not of age and therefore remained
          under the guardianship of his elder brother Jan Vilém. The widow at
          once dismissed Jan Kautský, who had been burgrave there for thirteen
          years; when Kautský secured a position with Diviš of Helversen at
          Krakovec, Jan Vilém refused to release his sick wife Barbora with her
          two children and had them imprisoned.
          <Qt
            publication={PUBLICATIONS.KOCKA1}
            href="https://kramerius5.nkp.cz/view/uuid:cbd24070-4608-11ed-b64c-005056827e52?page=uuid:982a126d-a074-4939-802f-22a8843d76ad"
            note="p. 316–317"
          />
        </p>
        <p>
          Because of their great debts the Týřovský brothers sold the Chříč
          estate in 1701 to Count Václav Josef Lažanský of Buková at Manětín for
          211,000 guilders and 300 guilders of key money; elsewhere Kočka gives
          the price as 211,500 guilders. The deed of sale names the village of
          Chříč with the manor house and two farmsteads, a brewery, a granary,
          the town of Kožlany, the villages of Studená, Hlince, Třímaný,
          Holovousy, Hedčany, Milíčov, Slatina and Lhota, the manors with
          sheepfolds of Ptíč, Březsko and Dubjany, and a mill with a powder
          house and a vineyard near the town of Louny. Each of the brothers
          reserved five persons for himself, and all of them agreed to pay their
          mother Kateřina Magdalena 300 guilders a year. Michal Antonín, the
          last holder of the Chříč inheritance, died in 1705; his wife was
          Konstancie of Šporek.
          <Qt
            publication={PUBLICATIONS.KOCKA1}
            href="https://kramerius5.nkp.cz/view/uuid:cbd24070-4608-11ed-b64c-005056827e52?page=uuid:cad91700-163b-46b7-9616-472c3b1b10de"
            note="p. 317, 361, 378"
          />
        </p>
      </Col>
    </Row>
  </>
)
