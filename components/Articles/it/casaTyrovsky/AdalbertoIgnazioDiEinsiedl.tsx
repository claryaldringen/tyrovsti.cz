import { Col, Row } from 'reactstrap'
import React from 'react'
import { PUBLICATIONS } from '../../../../shared/constants'
import { Qt } from '../../../Quote/Qt'

export const AdalbertoIgnazioDiEinsiedl = () => (
  <>
    <Row>
      <Col>
        <div className="anchor" id="adalberto-ignazio-di-einsiedl" />
        <h4>
          Vojtěch Ignác (Adalberto Ignazio) Týřovský di Einsiedl (morto nel
          1695) e la fine della famiglia a Chříč
        </h4>
        <p>
          Vojtěch Ignác ereditò da minorenne la tenuta di Chříč con la città di
          Kožlany, gravata da considerevoli debiti; gli rimase tutrice la madre
          Eva Eurosina, nata di Klenové. Questa nel 1655 inviò
          all&apos;arcivescovo un attestato secondo cui tutta la sua famiglia e
          tutti i sudditi di Kožlany erano di religione cattolica e che il
          lunedì di Pasqua avevano assistito alla funzione divina e alla predica
          tenuta dal parroco di Čistá Jan Procerius, che amministrava anche la
          parrocchia di Kožlany.
          <Qt
            publication={PUBLICATIONS.KOCKA1}
            href="https://kramerius5.nkp.cz/view/uuid:cbd24070-4608-11ed-b64c-005056827e52?page=uuid:b80db396-2bc6-4d88-ae38-d19072b4ef54"
            note="p. 377"
          />
        </p>
        <p>
          Raggiunse la maggiore età nel 1662 e prese egli stesso possesso
          dell&apos;eredità paterna. Oto Jiří di Helversen, signore di Krakovec,
          si assicurò subito sulla sua tenuta 3000 fiorini che il giovane
          Týřovský gli era rimasto debitore per oggetti d&apos;oro, diamanti e
          rubini. Nel 1676 acquistò il villaggio di Třímaný e nello stesso anno
          licenziò dal servizio il burgravio Daniel Vlašimský, in precedenza
          borghese praghese, al quale rimase debitore di 500 fiorini.
          <Qt
            publication={PUBLICATIONS.KOCKA1}
            href="https://kramerius5.nkp.cz/view/uuid:cbd24070-4608-11ed-b64c-005056827e52?page=uuid:982a126d-a074-4939-802f-22a8843d76ad"
            note="p. 316, 378"
          />
        </p>
        <p>
          Cercò di ridurre gli abitanti di Kožlany alla completa sottomissione:
          si fece consegnare da loro tutti i privilegi per prenderne visione e,
          quando dopo alcuni anni non li restituiva, i borghesi compresero
          l&apos;astuzia del signore e si lamentarono presso la camera boema.
          Per ordine del capitano regionale restituì loro le concessioni nel
          1680. Sotto il suo dominio sette sudditi di Kožlany abbandonarono i
          propri poderi e fuggirono. Quando nel 1680 gli abitanti di Čistá
          rivendicarono fiere annuali e dazio, quelli di Kožlany si opposero sia
          alle fiere sia al dazio, e il loro signore ereditario sosteneva che il
          monte Břežanská hora apparteneva a lui, poiché i suoi antenati lo
          avevano comprato da Albrecht Jindřich di Kolovrat.
          <Qt
            publication={PUBLICATIONS.KOCKA1}
            href="https://kramerius5.nkp.cz/view/uuid:cbd24070-4608-11ed-b64c-005056827e52?page=uuid:1ab20b93-66e8-48df-ae4b-67177e424fcd"
            note="p. 378, 473"
          />
        </p>
        <p>
          Alla chiesa di Milíčov, dove giaceva suo nonno, fece aggiungere nel
          1665 una torre e provvide un altare.
          <Qt
            publication={PUBLICATIONS.KOCKA1}
            href="https://kramerius5.nkp.cz/view/uuid:cbd24070-4608-11ed-b64c-005056827e52?page=uuid:6bf7328e-a9a9-4cb4-b5fc-b8c991028766"
            note="p. 411"
          />
        </p>
        <p>
          Morì nel 1695. Gli sopravvissero la vedova Kateřina Magdalena, nata
          contessa di Brandenburk, i figli Jan Vilém, Michal Antonín e František
          Pavel e le figlie Anna e Polyxena. Chříč fu ereditata da Michal
          Antonín, che non aveva ancora la maggiore età e rimase perciò sotto la
          tutela del fratello maggiore Jan Vilém. La vedova licenziò subito Jan
          Kautský, che era stato per tredici anni burgravio; quando Kautský si
          procurò un posto presso il signore Diviš di Helversen a Krakovec, Jan
          Vilém non volle lasciar partire la moglie malata Barbora con i due
          figli e li fece imprigionare.
          <Qt
            publication={PUBLICATIONS.KOCKA1}
            href="https://kramerius5.nkp.cz/view/uuid:cbd24070-4608-11ed-b64c-005056827e52?page=uuid:982a126d-a074-4939-802f-22a8843d76ad"
            note="p. 316–317"
          />
        </p>
        <p>
          A causa dei grandi debiti i fratelli Týřovský vendettero nel 1701 la
          tenuta di Chříč a Václav Josef conte Lažanský di Buková, signore di
          Manětín, per 211 000 fiorini e 300 fiorini per la consegna delle
          chiavi; in un altro punto Kočka indica il prezzo di 211 500 fiorini.
          Il contratto di vendita nomina il villaggio di Chříč con la residenza
          signorile e due poderi, il birrificio, il granaio, la città di
          Kožlany, i villaggi di Studená, Hlince, Třímaný, Holovousy, Hedčany,
          Milíčov, Slatina e Lhota, i poderi con gli ovili Ptíč, Březsko e
          Dubjany e il mulino con la polveriera e la vigna presso la città di
          Louny. Ciascuno dei fratelli si riservò cinque persone e tutti si
          impegnarono a pagare alla madre Kateřina Magdalena 300 fiorini
          all&apos;anno. Michal Antonín, ultimo detentore dell&apos;eredità di
          Chříč, morì nel 1705; sua moglie fu Konstancie di Špork.
          <Qt
            publication={PUBLICATIONS.KOCKA1}
            href="https://kramerius5.nkp.cz/view/uuid:cbd24070-4608-11ed-b64c-005056827e52?page=uuid:cad91700-163b-46b7-9616-472c3b1b10de"
            note="p. 317, 361, 378"
          />
        </p>
      </Col>
    </Row>
  </>
)
